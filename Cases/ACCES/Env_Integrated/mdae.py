...
# Add emissions block
x.add_system("Ems", EMISSIONS, r"\text{Environment}")

# Add functions with environment block
x.add_system("FuncE", FUNF, r"\text{Contrails}", stack=True)

# Add functions with environment block
x.add_system("FuncW", FUNF, r"\text{ERA-5}", stack=True)
...
# Environment module sends state to contrails function
x.connect("Ems", "FuncE", "u_e^*")

# Contrails function returns state to environment module
x.connect("FuncE", "Ems", "RF, emissions")

x.connect("FuncW", "FuncE", "ISSR")

x.connect("FuncW", "MDA", "wind")
